...
// Genera embeddings locales. Devuelve vectores de DIMENSION_EMBEDDING dimensiones.
// Si el modelo no se puede cargar, devuelve null (best-effort, nunca rompe el pipeline).
  ...
  try {
    ...
    for (const texto of textos) {
      try {
        ...
        // output es un Tensor; .tolist() devuelve number[] o number[][]
        const list = output.tolist();
        const vec = Array.isArray(list[0]) ? list[0] : list;
        // Aseguramos que sean números planos
